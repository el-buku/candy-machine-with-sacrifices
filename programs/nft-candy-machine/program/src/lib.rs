...
use {
    crate::utils::{
        ...
        assert_valid_go_live, spl_token_burn, spl_token_transfer, spl_token_transfer_checked,
        TokenBurnParams, TokenTransferCheckedParams, TokenTransferParams,
    },
    anchor_lang::{
        prelude::*,
        solana_program::{
...
anchor_lang::declare_id!("DRzAC5TAC3JrDt6NT9acLVLhoon4fF88WuCVszcNvXJ9");
...
pub mod nft_candy_machine_v2 {
    ...
    use super::*;
    pub fn confirm_sacrifice(ctx: Context<ConfirmSacrifice>) -> ProgramResult {
        let candy_machine = &mut ctx.accounts.candy_machine;
        let wallet = &ctx.accounts.wallet;
        let token_acc1_info = &ctx.accounts.sacrifice_token_address1.to_account_info();
        let token_mint1 = &ctx.accounts.sacrifice_token_mint1;
        let token_metadata1 = &ctx.accounts.sacrifice_token_metadata1;
        let token_acc2_info = &ctx.accounts.sacrifice_token_address2.to_account_info();
        let token_mint2 = &ctx.accounts.sacrifice_token_mint2;
        let token_metadata2 = &ctx.accounts.sacrifice_token_metadata2;
        let sacrifice_confirmation_account = &mut ctx.accounts.sacrifice_confirmation_account;
        msg!("here2");

        // sacrifice validation
        match assert_is_ata(token_acc1_info, &wallet.key(), &token_mint1.key()) {
            Ok(token_acc1) => {
                if token_acc1.amount < 1 {
                    return Err(ErrorCode::TokenNotSacrificed.into());
                }
                let (metadata1_address, _) = find_metadata_account(&token_mint1.key());
                if metadata1_address != token_metadata1.key() {
                    return Err(ErrorCode::WrongSacrificeAddress.into());
                }
                if Metadata::from_account_info(&token_metadata1.to_account_info())?
                    .data
                    .symbol
                    .trim()
                    .eq(candy_machine.data.sacrificed_token_symbol.trim())
                {
                    msg!("plmerr");
                    return Err(ErrorCode::WrongSacrificeAddress.into());
                }
            }
            Err(_) => return Err(ErrorCode::WrongSacrificeAddress.into()),
        }
        msg!("here3");

        match assert_is_ata(token_acc2_info, &wallet.key(), &token_mint2.key()) {
            Ok(token_acc2) => {
                if token_acc2.amount < 1 {
                    return Err(ErrorCode::TokenNotSacrificed.into());
                }
                let (metadata2_address, _) = find_metadata_account(&token_mint2.key());
                if metadata2_address != token_metadata2.key() {
                    return Err(ErrorCode::WrongSacrificeAddress.into());
                }
                if Metadata::from_account_info(&token_metadata2.to_account_info())?
                    .data
                    .symbol
                    .eq(&candy_machine.data.sacrificed_token_symbol)
                {
                    msg!("plmer2");
                    return Err(ErrorCode::WrongSacrificeAddress.into());
                }
        ...
        sacrifice_confirmation_account.confirmed = true;
        msg!("sacrifice confirmed");
        Ok(())
    }
    pub fn confirm_sacrifice_and_mint<'info>(
        ctx: Context<'_, '_, '_, 'info, MintNFTWithSacrifices<'info>>,
        creator_bump: u8,
        _confirmation_bump: u8,
    ) -> ProgramResult {
        msg!("here1");
        let candy_machine = &mut ctx.accounts.candy_machine;
        ...
        // Candy machine wallet
        let wallet = &ctx.accounts.wallet;
        let sacrifice_confirmation_account = &mut ctx.accounts.sacrifice_confirmation_account;
        msg!("here4");

        // mint nft when end_setting.number==0 but sacrifice check passed
        if !sacrifice_confirmation_account.confirmed {
            msg!("not confirmed");
            return Err(ErrorCode::TokenNotSacrificed.into());
        }
        sacrifice_confirmation_account.confirmed = false;
        msg!("here5");
...
#[derive(Accounts)]
#[instruction(_confirmation_bump: u8)]
pub struct ConfirmSacrifice<'info> {
    #[account(
        mut,
        has_one = wallet
    )]
    ...
    #[account(mut)]
    wallet: UncheckedAccount<'info>,
    sacrifice_token_mint1: UncheckedAccount<'info>,
    sacrifice_token_metadata1: UncheckedAccount<'info>,
    sacrifice_token_address1: UncheckedAccount<'info>,
    sacrifice_token_mint2: UncheckedAccount<'info>,
    sacrifice_token_metadata2: UncheckedAccount<'info>,
    sacrifice_token_address2: UncheckedAccount<'info>,
    #[account(init_if_needed, seeds=[PREFIX.as_bytes(), b"confirm",candy_machine.key().as_ref(), payer.key().as_ref()], bump=_confirmation_bump, payer=payer)]
    sacrifice_confirmation_account: Account<'info, Confirmation>,
    payer: Signer<'info>,
    system_program: Program<'info, System>,
}

#[derive(Accounts)]
#[instruction(creator_bump: u8, _confirmation_bump:u8)]
pub struct MintNFTWithSacrifices<'info> {
    #[account(
        mut,
        has_one = wallet
    )]
...
pub struct CandyMachineData {
    pub accepts_sacrifices: bool,
    pub sacrificed_token_symbol: String,
    pub uuid: String,
    pub price: u64,
